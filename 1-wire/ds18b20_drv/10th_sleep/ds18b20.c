...
#define DS18B20_RESET_ERROR		-1	//初始化失败
#define DS18B20_CRC_ERROR			-2	//CRC错误

//驱动模块名称
#define DEVICE_NAME "DS18B20"
//系统US延时定义
#define Delay_US(x)		udelay(x)
#define Sleep_MS(x)		msleep(x)
#define DS18B20_PIN		S3C2410_GPH12
...
{
	int bXorValue;	//
	unsigned char ucTempValue;		//模拟CRC硬件内部的8位数据寄存器unsigned charucCount;
	unsigned char ucCount;
	for(ucCount = 0;ucCount < 8;ucCount++)
	{
		//提取新数据位与CRC寄存器的内容的异或值的D0位作为内部通道的输入
		bXorValue = (ucNewData ^ ucCRCValue) & 0x01;//执行X4、X5的异或操作
		if(bXorValue)
		{
...
//入口参数：待校验的数据缓冲区和数据长度//测试数据，取自盛唐EPS11仿真编程器
		...
		loff_t *loff)
{
	...
	if(down_interruptible(&DS18B20_sem))		//获取信号量
		return -ERESTARTSYS;
	ret = DS18B20_ReadTemper(&temp);			//读取温度
	if(ret)				//DS18B20初始化失败
	{
