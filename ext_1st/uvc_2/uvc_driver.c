...
static struct uvc_format_desc uvc_fmts[] = {
	...
	#endif
	{
		.name		= "Greyscale (16-bit)",
		.guid		= UVC_GUID_FORMAT_Y16,
		.fcc		= v4l2_fourcc('Y', '1', '6', ' '), /* 16  Greyscale     */
	},
	/* 新修改的 */
	{
		.name		= "RGB Bayer",
		...
		struct uvc_streaming *stream)
{
	...
	/* 新增加的 */
	printk("uvc_register_video start\n");
//	printk("dev->name    = %s\n", dev->name);
	/* 新增加的 结束*/
...
struct uvc_driver uvc_driver = {
	.driver = {
		...
		.suspend	= uvc_suspend,
		.resume		= uvc_resume,
//		.reset_resume	= uvc_reset_resume,		/* 新修改的 */
		.id_table	= uvc_ids,
