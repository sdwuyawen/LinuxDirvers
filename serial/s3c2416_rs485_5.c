...
#define DEBUG    1	/* 这里是为了打印出printk(KERN_DEBUG...)等级的信息 */
...
/********* add here *********/
#include <asm-arm/plat-s3c24xx/clock.h>	/* 为calc->src->name结构添加  */
/********* end add *********/
				   ...
				   unsigned int baud)
{
	...
	rate = clk_get_rate(calc->src);	/* calc->src是一个struct clk *，rate是0x3FCF3，即261363 */
								/* 	正确的CLK是这样的: 12,2,1
									FCLK:      800 MHz
									HCLK:  133.333 MHz
									PCLK:   66.667 MHz */	
	...
	putstring_VA("clksrc->divisor=");	/* clksrc->divisor是1 */
...
#else
	/* Error correction */
	calc->quot = (rate) / (16 * baud);	/* 获取UART分频系数=pclk/(16*baud)，获取的pclk是错误的 */
#endif								
					  ...
					  unsigned int baud)
{
	...
	if (cfg->clocks_size < 2) {			/* 进入这里 */
		if (cfg->clocks_size == 0)
			clkp = &tmp_clksrc;		/* 进入这里 */

		/* check to see if we're sourcing fclk, and if so we're
		 * going to have to update the clock source
		 */

		if (strcmp(clkp->name, "fclk") == 0) {	/* clkp->name="pclk",不进入这里 */
		...
		s3c24xx_serial_calcbaud(res, port, clkp, baud);	/* 填充值错误!!!填充res[]。port是s3c24xx_serial_ports[0] */
		best = res;
		resptr = best + 1;
	} else {			/* 不进入这里 */
		resptr = res;
...
//	putinthex_VA(baud);
	...
	if (baud == 38400 && (port->flags & UPF_SPD_MASK) == UPF_SPD_CUST)
		...
		quot = s3c24xx_serial_getclk(port, &clksrc, &clk, baud);	/* !!执行这里，返回的quot有问题 */
	...
	putstring_VA("quot=");		/* 0xFFFFFFFF，这里有问题!!!!!!*/
	putinthex_VA(quot);
	...
	wr_regl(port, S3C2410_ULCON, ulcon);	/* 把计算出的寄存器值写入到寄存器 */
...
static struct uart_ops s3c24xx_serial_ops = {
	...
	.startup	= s3c24xx_serial_startup,				/* 对应用户空间的open操作 */
...
/* UART设备驱动，关键数据结构3:
 * uart_port，封装在私有类型struct s3c24xx_uart_port中
 */
static struct s3c24xx_uart_port s3c24xx_serial_ports[NR_PORTS] = {
	[0] = {
		.port = {
//			.lock		= __SPIN_LOCK_UNLOCKED(s3c24xx_serial_ports[0].port.lock),
			.lock		= SPIN_LOCK_UNLOCKED,		/* 复制的helper2416, 2.6.21 */
			.iotype		= UPIO_MEM,
			.irq		= IRQ_S3CUART_RX0,
			.uartclk	= 0,
			.fifosize	= 16,
				     ...
				     struct s3c24xx_uart_clksrc *clk)
{
	...
	else if (strcmp(clk->name, "esysclk") == 0)	/* 原来是fclk */
...
static int __init s3c24xx_serial_modinit(void)
{
	...
	s3c2440_serial_init();									/* 注册平台驱动，probe函数中有uart_add_one_port() */
...
s3c24xx_serial_console_setup(struct console *co, char *options)
{
	...
	return uart_set_options(port, co, baud, parity, bits, flow);	/* 卡在这里面了 */
}
...
static struct console s3c24xx_serial_console =
{
	.name		= S3C24XX_SERIAL_NAME,		//会生成/proc/tty/driver/ttySAC
	.device		= uart_console_device,
	.flags		= CON_PRINTBUFFER,
	.index		= -1,						//使用哪个串口由控制台参数决定
...
static int s3c24xx_serial_initconsole(void)
{
	...
	if (strcmp(dev->name, "s3c2440-uart") == 0) {	/* 进入这里 */
	...
	register_console(&s3c24xx_serial_console);	/* 没有从这里返回，或者从这之后串口已经无法打印 */
