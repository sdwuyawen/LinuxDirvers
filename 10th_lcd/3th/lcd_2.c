...
/* LCD控制器配置数据 */
#define FRAME_BUFFER   		0x32f00000	/* 显存地址，需要修改 */
#define ROW					272
#define COL					480
#define HSPW 				(41 - 1)
#define HBPD			 	(2- 1)
...
// 清屏
void lcd_clear_screen(int color, unsigned long ptr)
{
#if 0
	int i, j;
	for (i = 0; i < ROW; i++)
	...
	int i;
	unsigned long * pixel = (unsigned long  *)ptr;
...
#define RightBotY   271
	...
	*VIDW00ADD1B0 = s3c_lcd->fix.smem_start + s3c_lcd->fix.smem_len;
	...
	lcd_clear_screen(((0xFF >> 3) << 11)|((0x00 >> 2)  << 5)|((0x00 >> 3)  << 0), s3c_lcd->screen_base);		// 绿色  								// 黑色
