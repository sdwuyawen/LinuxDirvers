...
unsigned char  spi_hw_send_byte(unsigned char value)
{
	...
	/* 等待TX FIFO空 */
	/* 刚把数据写入TX FIFO时，S3C_SPI_STATUS的bit21是0，只有TX FIFO的数据写入
	 * 移位寄存器后，S3C_SPI_STATUS.bit21才会变1。所以先判断TX FIFO的数量，再判断bit21
	 */
	while((S3C_SPI_STATUS & (0x7F << 6)) != 0)
	{
...
//	while((S3C_SPI_STATUS & (0x7F << 13)) == 0)
//	{
//	
//	}
